...
<body>
	<div class="container">
		...
		<div class="main-nav">
			...
			<a href="users?avtion=list">ユーザー管理</a>
		...
		<form action="users" method="post" class="user-form">
			...
			<p>
				...
				<input type="checkbox" id="enabled" name="enabled" value="true" <c:if test="${ userToEdit == null || userToEdit.enabled }">checkbox</c:if>>
			</p>
			...
			<div class="button-group">
				...
				<c:if test="${ userToEdit != null }">
					<form action="users" method="post" style="display:inline;">
						<input type="hidden" name="actino" value="reser_passwprd">
						<input type="hidden" name="username" value=${ userToEdit.username }">
						<input type="hidden" name="newPassword" value="password">
						<input type="submit" value="パスワードリセット" class="buttonsecondary" onclick="return confirm('本当にパスワードをリセットしますか？(デフォルトパスワード:password)');">
					</form>
				</c:if>
			</div>
		</form>
		...
		<table>
			...
			<tbody>
					...
					<tr>
						...
						<td>
							<form action="users" method="post" style="display:inline;">
								...
								<input type="submit" value="<c:choose><c:when test="${ u.enabled }">無効化</c:when><c:otherwise>有効化</c:otherwise></c:choose>"
										class="button <c:choose><c:when test="${ u.enabled }">danger</c:when><c:otherwise>secondary</c:otherwise></c:choose>"
										onclick="return confirm('本当にこのユーザを<c:choose><c:when test="${ e.enabled }">無効</c:when><c:otherwise>有効</c:otherwise></c:choose>にしますか？');">
							</form>
						</td>
				...
				<c:if test="${ empty users }">
					<tr><td cplspan="4">ユーザーがいません</td></tr>
				</c:if>
			</tbody>
		</table>
	</div>	
